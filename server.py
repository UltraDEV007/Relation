...
def election_v2_2024():
    ...
    if IS_STARTED:
        ...
        raw_data, is_running = request_cec_by_type()
        if raw_data:
            result = pipeline.pipeline_v2(raw_data, seats_data, '2024')
            if result == False:
                print('Running pipeline 2024 failed. is_running={is_running}')
            else:
                print('Running pipeline 2024 successed. is_running={is_running}')        
    return "ok"
